...
export const handler = async (event) => {
  ...
  try {
    ...
    // 2. SCAVENGE FOR DELETED ITEMS
    const activePois = [];
    const deletedids = [];

    // 🔥 DUMMY TEST: Always add one ID to prove the app can see them 🔥
    deletedids.push("999999");

    allPois.forEach(poi => {
      // Find the POI ID (case-insensitive search)
      const poiIdKey = Object.keys(poi).find(k => k.toLowerCase() === 'poiid');
      const id = poi[poiIdKey];
      if (!id) return;

      // 🔥 AGGRESSIVE SCAVENGER 🔥
      // Look at EVERY key in the object. If ANY key containing "deleted" is true, mark it.
      let isDeleted = false;
      for (const key in poi) {
        if (key.toLowerCase().includes("deleted")) {
          const val = poi[key];
          if (val === true || 
              val === 1 || 
              val === "1" || 
